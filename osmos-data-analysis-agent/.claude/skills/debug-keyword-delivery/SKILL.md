...
Before `lookup_campaign`, ASK which ID type the IDs are: "Is [ID] a
...
`check_targeted_keyword_performance_in_campaigns` **twice in parallel** (current +
...
Optional zoom-out: `get_merchant_keyword_performance(client_ids=[advertiser
os_client_id], + baseline)` = ALL the merchant's targeted keywords across ALL its
...
route). `get_merchant_category_performance` = the same by category × campaign.
...
`check_keyword_request_volume` with the keyword(s) + the current period's
...
client_ids, marketing_campaign_ids, start/end, search_queries=[<kw>])`.
...
**6b. Competitor-campaign view** — `get_targeted_keyword_competition(agency_id,
search_queries=[<kw>], + baseline, exclude_marketing_campaign_ids=[user's])` in
...
campaign on the RIGHT bid metric (read `bidding_strategy` from `lookup_campaign`:
...
bid and won't appear there. `get_search_query_performance(search_queries=[<kw>],
breakdown_by="campaign", + baseline)` MARKETPLACE-WIDE (no `marketing_campaign_ids`)
...
`get_campaign_status_changes` on the user's campaign (recent pause/edit) + suggest
